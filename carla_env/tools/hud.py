...
class HUD(object):
    ...
    def render(self, display, extra_info=[], fear_score=0.0):
        # Render fear score at top with color
        self._render_fear_score(display, fear_score)
    ...
    def _render_fear_score(self, display, fear_score):
        """Render fear score at the top with color coding"""
        # Determine color based on fear level
        if fear_score < 0.3:
            color = (0, 255, 0)      # Green - Safe
        elif fear_score < 0.5:
            color = (255, 255, 0)    # Yellow - Caution
        elif fear_score < 0.7:
            color = (255, 165, 0)    # Orange - Moderate Risk
        else:
            color = (255, 0, 0)      # Red - High Risk/Danger
        
        # Create fear score text
        fear_text = f"FEAR: {fear_score:.3f}"
        
        # Use larger font for visibility - use default font since font_mono doesn't have get_name()
        font_large = pygame.font.Font(pygame.font.get_default_font(), 24)
        text_surface = font_large.render(fear_text, True, color)
        text_rect = text_surface.get_rect()
        
        # Position at top center
        text_rect.centerx = display.get_width() // 2
        text_rect.y = 10
