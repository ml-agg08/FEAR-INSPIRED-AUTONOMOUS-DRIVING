        ...
        for k, v in data.items():
            if isinstance(v, str) and v.isnumeric():
                ...
                for k_inner, v_inner in v.items():
                    try:
                        config_dict[k][k_inner] = str(v_inner)
                    except:
                        config_dict[k][k_inner] = type(v_inner).__name__
                config_dict[k] = str(config_dict[k])
            else:
                try:
                    config_dict[k] = str(v)
                except:
                    config_dict[k] = type(v).__name__
    ...
    def _on_training_start(self) -> None:
        ...
        for k, v in self.config.items():
            if isinstance(v, str) and v.isnumeric():
                ...
                for k_inner, v_inner in v.items():
                    try:
                        hparam_dict[k][k_inner] = str(v_inner)
                    except:
                        hparam_dict[k][k_inner] = type(v_inner).__name__
                hparam_dict[k] = str(hparam_dict[k])
            else:
                try:
                    hparam_dict[k] = str(v)
                except:
                    hparam_dict[k] = type(v).__name__
